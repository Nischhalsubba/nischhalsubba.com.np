...
if (styleSources.some(/** Callback contract: Evaluate whether the current item satisfies the enclosing existential condition. Inputs: `file`. Side effects: reads filesystem state. Returns: boolean predicate result. */ (file) => !fs.existsSync(file)) || !fs.existsSync(distStyle)) {
