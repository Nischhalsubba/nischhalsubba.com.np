...
for (const [viewportName, viewport] of viewports) {
  ...
  for (const [routeName, routePath] of routes) {
    const name = `${routeName}-${viewportName}.png`;
...
console.log(`[visual-regression] ${routes.length * viewports.length} snapshots ${update ? 'updated' : 'passed'} at ${(maximumDifferenceRatio * 100).toFixed(2)}% tolerance.`);
