...
if (!fs.existsSync(partsDir)) throw new Error(`[signal-source-preflight] Missing ${partsDir}`);
const discovered = fs.readdirSync(partsDir).filter((name) => name.endsWith('.b64part')).sort((a, b) => a.localeCompare(b, 'en', { numeric: true }));
const expectedNames = EXPECTED_PARTS.map(([name]) => name).sort((a, b) => a.localeCompare(b, 'en', { numeric: true }));
if (JSON.stringify(discovered) !== JSON.stringify(expectedNames)) throw new Error(`[signal-source-preflight] Segment set mismatch. Expected ${expectedNames.join(', ')}, found ${discovered.join(', ')}.`);
const encoded = EXPECTED_PARTS.map(([name, expectedChars, expectedHash]) => {
  ...
  if (value.length !== expectedChars) throw new Error(`[signal-source-preflight] ${name} character count mismatch. Expected ${expectedChars}, found ${value.length}.`);
  const actualHash = sha256(value);
  if (actualHash !== expectedHash) throw new Error(`[signal-source-preflight] ${name} checksum mismatch. Expected ${expectedHash}, found ${actualHash}.`);
  return value;
}).join('');
const poster = Buffer.from(encoded, 'base64');
if (poster.length !== EXPECTED_POSTER_BYTES) throw new Error(`[signal-source-preflight] Poster byte count mismatch. Expected ${EXPECTED_POSTER_BYTES}, found ${poster.length}.`);
const posterHash = sha256(poster);
if (posterHash !== EXPECTED_POSTER_SHA256) throw new Error(`[signal-source-preflight] Poster checksum mismatch. Expected ${EXPECTED_POSTER_SHA256}, found ${posterHash}.`);
if (poster.subarray(0, 4).toString('ascii') !== 'RIFF' || poster.subarray(8, 12).toString('ascii') !== 'WEBP') throw new Error('[signal-source-preflight] Verified bytes are not a WebP container.');
