...
const files=fs.existsSync(root)?fs.readdirSync(root).filter( /** Callback contract: Decide whether the current item remains in the filtered result consumed by the enclosing operation. Inputs: `n` Side effects: No direct external side effect beyond invoked dependencies. Returns: Boolean predicate result consumed by the enclosing collection lookup/filter. */ n=>/^project-[a-z0-9-]+\.html$/i.test(n)&&!skip.has(n)):[];
 ...
 */
function cls(h,t,c){return h.replace(new RegExp(`<${t}([^>]*)>`,'i'),   /** Callback contract: Perform the local callback step required by the immediately enclosing normalize case study system repository tool operation. Inputs: `m`, `a` Side effects: No direct external side effect beyond invoked dependencies. Returns: Computed result consumed by the caller; explicit early-return branches define fallback behavior. */ (m,a)=>{const f=a.match(/class=["']([^"']*)["']/i);if(!f)return `<${t}${a} class="${c}">`;const s=new Set(f[1].split(/\s+/).filter(Boolean));s.add(c);return `<${t}${a.replace(f[0],`class="${[...s].join(' ')}"`)}>`})}
