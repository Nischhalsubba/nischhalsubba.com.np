...
function updateThemeButton(button, theme, isManual) {
  if (!button) return;
  ...
  button.setAttribute('title', isManual ? `Manual ${theme} theme` : `Using system ${theme} theme`);
}

function applyTheme(theme, button, isManual = false) {
  const nextTheme = theme === 'light' ? 'light' : 'dark';
  document.documentElement.setAttribute('data-theme', nextTheme);
  document.documentElement.style.colorScheme = nextTheme;
  updateThemeButton(button, nextTheme, isManual);
...
export function initTheme() {
  ...
  applyTheme(hasManualTheme ? savedTheme : systemTheme(), button, hasManualTheme);
