...
function removeDuplicateFloatingResumeControls() {
  ...
  floatingControls.slice(1).forEach( /** Callback contract: Apply the enclosing side-effect operation to the current collection item. Inputs: `control` Side effects: No direct external side effect beyond invoked dependencies. Returns: Undefined; this callback is side-effect-only. */ (control) => control.remove());
}
