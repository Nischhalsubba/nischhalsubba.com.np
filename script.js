...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function setTheme(theme) {
        ...
        const targets = document.querySelectorAll('.hero-portrait-img, .footer-portrait-img, .profile-img');
        targets.forEach(img => img.src = theme === 'light' ? LIGHT_IMG : DARK_IMG);
    }
    ...
    if (glider) {
        const moveGlider = (el) => {
            gsap.to(glider, { x: el.offsetLeft, width: el.offsetWidth, opacity: 1, duration: 0.3 });
        };
    ...
    const setupSearch = (inputId, clearId, itemsClass, categoryAttr) => {
        ...
        const filter = () => {
            ...
            items.forEach(item => {
                ...
                const tags = item.getAttribute(categoryAttr) || "";
                const matches = text.includes(val) || tags.toLowerCase().includes(val);
                item.style.display = matches ? (itemsClass.includes('writing') ? 'grid' : 'flex') : 'none';
                if(matches) gsap.fromTo(item, {opacity:0, y:10}, {opacity:1, y:0, duration:0.3});
            });
        };
    ...
    if (!REDUCED_MOTION && window.gsap && window.ScrollTrigger) {
        ...
        document.querySelectorAll('.text-reveal-wrap').forEach(title => {
            ...
            if(fillText) gsap.to(fillText, { clipPath: 'inset(0 0% 0 0)', scrollTrigger: { trigger: title, start: 'top 85%', scrub: 1 } });
        });
        document.querySelectorAll(".reveal-on-scroll").forEach(el => {
            gsap.fromTo(el, {y:30, opacity:0}, { y:0, opacity:1, duration:0.8, scrollTrigger: { trigger: el, start: "top 85%" } });
        });
    }
